...
import { ArrowLeft, Download, FileType, Loader2, FileText, AlertCircle, X } from 'lucide-react';
import { Link } from 'react-router-dom';
import FileUploader from '../components/FileUploader';
import { convertWordToPdf, downloadPdf } from '../services/pdfService';
...
const WordToPdf: React.FC = () => {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      setPdfBlob(blob);
      downloadPdf(blob, `${file.file.name.replace(/\.(docx|doc)$/, '')}.pdf`);
      setIsDone(true);
    } catch (error: any) {
      console.error(error);
      setErrorMessage(error.message || "Error converting Word to PDF.");
    } finally {
  ...
  const handleDownload = () => {
    if (pdfBlob && file) {
      downloadPdf(pdfBlob, `${file.file.name.replace(/\.(docx|doc)$/, '')}.pdf`);
    }
  };
          ...
          <div className="max-w-xl w-full">
            ...
            <p className="text-center mt-6 text-gray-500">
             ...
             <div className="w-full max-w-2xl">
                <div className="text-center py-10 bg-white rounded-xl shadow-sm border border-gray-100 mb-8">
                    ...
                    <h2 className="text-3xl font-bold text-gray-900 mb-4">Word converted to PDF!</h2>
            ...
            <div className="w-full max-w-2xl bg-white p-8 rounded-xl shadow-sm border border-gray-200 flex flex-col items-center">
                 ...
                 <div className="w-full bg-gray-50 p-6 rounded-lg border border-gray-200 mb-8">
                    ...
                    <div className="flex items-center gap-3 p-3 bg-white border border-blue-200 rounded-lg shadow-sm">
                        <div className="p-2 bg-blue-50 text-blue-600 rounded-full">
                            <FileType size={20} />
                        </div>
                        <div className="flex-grow">
                            <p className="font-medium text-gray-900">Word to PDF</p>
                            <p className="text-xs text-gray-500">Convert DOCX content to portable PDF format.</p>
                        </div>
