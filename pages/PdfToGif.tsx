...
import { ArrowLeft, Download, Film, Loader2, AlertCircle, X, PlayCircle } from 'lucide-react';
import { Link } from 'react-router-dom';
import FileUploader from '../components/FileUploader';
import { convertPdfToGif, downloadPdf } from '../services/pdfService';
import { UploadedFile } from '../types';

const PdfToGif: React.FC = () => {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const blob = await convertPdfToGif(file.file, delay);
      setGifBlob(blob);
      setIsDone(true);
    } catch (error: any) {
      console.error(error);
      setErrorMessage(error.message || "Error converting PDF to GIF.");
    } finally {
  ...
  const handleDownload = () => {
    if (gifBlob && file) {
      downloadPdf(gifBlob, `${file.file.name.replace('.pdf', '')}.gif`);
    }
  };
    ...
    <div className="min-h-[calc(100vh-64px)] bg-gray-50 flex flex-col">
      ...
      <div className="bg-white border-b border-gray-200 py-4 px-4 sm:px-6 lg:px-8 flex items-center justify-between sticky top-16 z-40">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold text-gray-900">PDF to GIF</h1>
        </div>
        {file && !isDone && (
            <button
        ...
        {!file ? (
          <div className="max-w-xl w-full">
            ...
            <p className="text-center mt-6 text-gray-500">
                Turn your PDF pages into an animated GIF. Great for presentations and social media previews.
            </p>
          </div>
        ) : isDone ? (
             <div className="w-full max-w-3xl flex flex-col items-center">
                <div className="text-center py-10 bg-white rounded-xl shadow-sm border border-gray-100 mb-8 w-full">
                    <div className="inline-flex items-center justify-center p-4 bg-purple-100 text-purple-600 rounded-full mb-6">
                        <Film size={48} />
                    </div>
                    <h2 className="text-3xl font-bold text-gray-900 mb-4">GIF Created!</h2>
                    ...
                    {gifBlob && (
                        <div className="mb-8 flex justify-center">
                            <img 
                                ...
                                alt="Generated GIF" 
                                className="max-h-80 border rounded shadow-md"
                            />
                        </div>
                    )}
                    ...
                    <div className="flex flex-col sm:flex-row justify-center gap-4">
                        ...
                        >
                         ...
                         <button 
                            onClick={() => { setFile(null); setGifBlob(null); setIsDone(false); }} 
            ...
            <div className="w-full max-w-2xl bg-white p-8 rounded-xl shadow-sm border border-gray-200 flex flex-col items-center">
                 ...
                 <div className="w-full bg-gray-50 p-6 rounded-lg border border-gray-200 mb-8">
                    <h3 className="font-semibold text-gray-800 mb-4">Animation Settings</h3>
                    
                    <div className="mb-4">
                        <label className="flex items-center justify-between text-sm font-medium text-gray-700 mb-2">
                             <span>Frame Speed (Delay per page)</span>
                             <span className="text-red-600">{delay} ms</span>
                        </label>
                        <input 
                            type="range" 
                            min="100" 
                    ...
                    <div className="flex items-center gap-3 p-3 bg-white border border-purple-200 rounded-lg shadow-sm mt-4">
                        <div className="p-2 bg-purple-50 text-purple-600 rounded-full">
                            <PlayCircle size={20} />
                        </div>
                        <div className="flex-grow">
                            <p className="font-medium text-gray-900">Animated Loop</p>
                            <p className="text-xs text-gray-500">Creates an infinitely looping GIF from your pages.</p>
                        </div>
