...
import { ArrowLeft, Download, FileCode, Loader2, AlertCircle, X, Globe } from 'lucide-react';
...
const HtmlToPdf: React.FC = () => {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const blob = await convertHtmlToPdf(file.file);
      setPdfBlob(blob);
      downloadPdf(blob, `${file.file.name.replace(/\.html?$/i, '')}.pdf`);
      setIsDone(true);
    } catch (error: any) {
      console.error(error);
      setErrorMessage(error.message || "Error converting HTML to PDF.");
    } finally {
  ...
  const handleDownload = () => {
    if (pdfBlob && file) {
      downloadPdf(pdfBlob, `${file.file.name.replace(/\.html?$/i, '')}.pdf`);
    }
  };
    ...
    <div className="min-h-[calc(100vh-64px)] bg-gray-50 flex flex-col">
      ...
      <div className="bg-white border-b border-gray-200 py-4 px-4 sm:px-6 lg:px-8 flex items-center justify-between sticky top-16 z-40">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold text-gray-900">HTML to PDF</h1>
        </div>
        {file && !isDone && (
            <button
        ...
        {!file ? (
          <div className="max-w-xl w-full">
            ...
            <p className="text-center mt-6 text-gray-500">
                Convert webpages or HTML files to PDF documents.
            </p>
          </div>
        ) : isDone ? (
             <div className="w-full max-w-2xl">
                <div className="text-center py-10 bg-white rounded-xl shadow-sm border border-gray-100 mb-8">
                    ...
                    <h2 className="text-3xl font-bold text-gray-900 mb-4">HTML converted to PDF!</h2>
                 ...
                 <div className="w-full bg-gray-50 p-6 rounded-lg border border-gray-200 mb-8">
                    ...
                    <div className="flex items-center gap-3 p-3 bg-white border border-indigo-200 rounded-lg shadow-sm">
                        ...
                        <div className="flex-grow">
                            <p className="font-medium text-gray-900">HTML to PDF</p>
                            <p className="text-xs text-gray-500">Render HTML content as a PDF document.</p>
                        </div>
