...
import { ArrowLeft, Download, Globe, Loader2, AlertCircle, X, FileCode } from 'lucide-react';
import { Link } from 'react-router-dom';
import FileUploader from '../components/FileUploader';
import { convertPdfToHtml, downloadPdf } from '../services/pdfService';
...
const PdfToHtml: React.FC = () => {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const blob = await convertPdfToHtml(file.file);
      setHtmlBlob(blob);
      downloadPdf(blob, `${file.file.name.replace('.pdf', '')}.html`);
      setIsDone(true);
    } catch (error: any) {
      console.error(error);
      setErrorMessage(error.message || "Error converting PDF to HTML.");
    } finally {
  ...
  const handleDownload = () => {
    if (htmlBlob && file) {
      downloadPdf(htmlBlob, `${file.file.name.replace('.pdf', '')}.html`);
    }
  };
    ...
    <div className="min-h-[calc(100vh-64px)] bg-gray-50 flex flex-col">
      ...
      <div className="bg-white border-b border-gray-200 py-4 px-4 sm:px-6 lg:px-8 flex items-center justify-between sticky top-16 z-40">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold text-gray-900">PDF to HTML</h1>
        </div>
        {file && !isDone && (
            <button
            ...
            className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-white shadow-lg transition-all
                ${isProcessing ? 'bg-gray-400 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700 hover:scale-105'}
            `}
            >
            {isProcessing ? (
                <>
        ...
        {!file ? (
          <div className="max-w-xl w-full">
            ...
            <p className="text-center mt-6 text-gray-500">
                Convert your PDF files to HTML webpages.
            </p>
          </div>
        ) : isDone ? (
             <div className="w-full max-w-2xl">
                <div className="text-center py-10 bg-white rounded-xl shadow-sm border border-gray-100 mb-8">
                    ...
                    <h2 className="text-3xl font-bold text-gray-900 mb-4">PDF converted to HTML!</h2>
                    <p className="text-gray-600 mb-8">
                        Your HTML file is ready. The download should have started automatically.
                    </p>
                    <div className="flex flex-col sm:flex-row justify-center gap-4">
                        ...
                        >
                         ...
                         <button 
                            onClick={() => { setFile(null); setHtmlBlob(null); setIsDone(false); }} 
            ...
            <div className="w-full max-w-2xl bg-white p-8 rounded-xl shadow-sm border border-gray-200 flex flex-col items-center">
                 ...
                 <div className="w-full bg-gray-50 p-6 rounded-lg border border-gray-200 mb-8">
                    ...
                    <div className="flex items-center gap-3 p-3 bg-white border border-indigo-200 rounded-lg shadow-sm">
                        <div className="p-2 bg-indigo-50 text-indigo-600 rounded-full">
                            <Globe size={20} />
                        </div>
                        <div className="flex-grow">
                            <p className="font-medium text-gray-900">PDF to HTML</p>
                            <p className="text-xs text-gray-500">Convert document content to a webpage structure.</p>
                        </div>
                 ...
                 <button
                    onClick={handleConvert}
                    className="w-full bg-indigo-600 text-white py-4 rounded-xl font-bold text-lg shadow-md hover:bg-indigo-700 transition-transform hover:-translate-y-0.5"
                >
                    Convert to HTML
                </button>
            </div>
